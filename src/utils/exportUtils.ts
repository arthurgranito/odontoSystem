...
export const exportarConsultasExcel = (
  ...
  const dadosExport = consultas.map((consulta) => {
    const dados: any = {
      Data: formatarData(
        consulta.agendaDisponivel?.data || consulta.dataHoraInicio
      ),
    ...
    if (incluirHorario) {
      dados.Horário = converteHora(consulta.dataHoraInicio?.split("T")[1]);
    }

    // Se for faturamento, incluir valores
    if (consulta.status === "CONCLUIDA") {
      dados["Valor Cobrado"] =
        consulta.valorCobrado || consulta.tipoConsulta?.preco || 0;
